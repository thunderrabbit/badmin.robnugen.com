...
<body>
  ...
  <section id="result" class="hidden">
    <p class="ok">Filed! 🎉</p>
    <img id="resultThumb" alt="" style="max-width:200px;border-radius:8px">
...
photo.addEventListener('change', () => {
  ...
  nameBtn.disabled = !photo.files[0];
});
...
async function askNames(model) {
  ...
  if (model === 'haiku' || !state.token) {
    if (!photo.files[0]) { setStatus('Pick a photo first.', 'err'); return; }
    fd.append('photo', photo.files[0]);
  } else {
    fd.append('token', state.token);   // re-ask reuses the staged photo
  }
...
$('confirmBtn').onclick = async () => {
  ...
  try {
    ...
    $('resultThumb').src = j.url; $('resultMd').value = j.markdown || j.url;
...
$('nextBtn').onclick = () => {
  ...
  photo.value = ''; $('preview').style.display = 'none';
  $('name').value = ''; $('tags').value = ''; $('catNew').value = ''; $('aiDesc').textContent = '';
  $('nameChips').innerHTML = ''; nameBtn.disabled = true;
